...
- 🐛 **MySQL 遗留表字符集修复** — 新增 `V0008__fix_legacy_mysql_table_charsets`，将历史遗留的 `latin1` 表统一迁移到 `utf8mb4`，修复中文股票名、聊天记录、新闻摘要、分析结果 JSON 等内容写库失败的问题；同时下调 LiteLLM 调试日志等级，减少无效 debug 噪音。
...
- 🔧 **统一 `.env.example` 配置模板** — 将仓库内主要环境变量按模块集中到 `.env.example`，补充中文注释，并新增 `scripts/check_env_example_sync.py` 用于检查示例配置是否覆盖运行时 / 注册表配置项，减少新增配置后模板漏改的问题。

### 修复

- 📈 **stock_daily 批量补齐提速与续跑** — `stock_daily` 的手动历史回填与每日定时补齐都已改为按待处理 code 数量自动判断是否启用公共线程池；`tests/manual_stock_daily_sync_runner.py history-backfill` 不再默认从上市日/1990 年开始全量回填，而是先按 AkShare 交易日历计算最近 30 个交易日窗口，并支持 `--max-workers` 上限控制与 checkpoint 断点续跑，数据库或网络抖动后可直接续跑未完成 code。
